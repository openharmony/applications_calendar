 ...
 * 对数据库里的字段类型和内存里的字段类型不一致时,做转换
 * @param columnConfig
 * @returns
 */
export function ColumnMapper(rawColumnConfig: RawColumnConfig<keyof SupportedDbValueType>, mapper: Mapper<ESObject, ESObject>): Function {
  return function (target: Prototype, propertyName: string): void {
    ...
    ColumnMapperHelper.INSTANCE.fillColumnMapper(target, propertyName, columnConfig, mapper);
  };
}
